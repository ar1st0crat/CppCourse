...
void processArray(int a[], size_t len)
{
...
int main()
{
	...
	std::cout << *valuePtr << std::endl;	// операция * - разыменовать указатель (получить те данные в памяти, на которые он указывает)
	...
	*val = 35;				// присвоить по указателю данные 35
	...
	delete val;				// освободить в памяти HEAP (теперь утечки памяти не будет, когда в конце main() указатель хх будет уничтожен)
	...
	for (size_t i = 0; i < 5; ++i)		// пример другой инициализации - в цикле числами 14, 15, 16, 17, 18
	{                                   // NB. Тип size_t - в данном случае эквивалент unsigned int
		heapArray[i] = 14 + i;
	}
	...
	std::cout << sizeof(arr) / sizeof(arr[0]) << std::endl;      // так можно узнать размер массива на стеке

	processArray(arr, 10);				     // передаем массив в функцию (передача параметра по указателю - поэтому массив будет меняться)
	...
	{
		*(shortArray + i) = 14 + i;		// (эта запись эквивалентна shortArray[i] 
